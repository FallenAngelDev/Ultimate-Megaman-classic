...
func _on_player_detector_body_exited(_body: Megaman) -> void:
	shild_block = true
	$ThrowTimer.stop()
	sprite.play("idle")
	target = null
...
func _on_throw_timer_timeout() -> void:
	...
	var instante_gravity = pick_axe_instance.gravity
	pick_axe_instance.velocity = Vector2(pick_axe_instance.speed * distance.x/ instante_gravity , -pick_axe_instance.speed * 2)
	if distance.x > 0:
		sprite.flip_h = true
	else :
		sprite.flip_h = false
